...
def findservices(addr=None, name=None, servicetype=None):
    ...
    for devaddr in addresses:
        iobtdevice = _IOBluetooth.IOBluetoothDevice.withAddressString_(
            devaddr)
...
def finddevicename(address, usecache=True):
    ...
    device = _IOBluetooth.IOBluetoothDevice.withAddressString_(
                address)
    if usecache:
        name = device.getName()
        if name is not None:
...
def selectdevice():
    ...
    if response == AppKit.NSRunStoppedResponse:
        ...
        if len(results) > 0:  # should always be > 0, but check anyway
            ...
            # it        
            dev = _IOBluetooth.IOBluetoothDevice.withAddressString_(
                        devinfo[0])
            if dev.isConnected(): 
                dev.closeConnection()        
